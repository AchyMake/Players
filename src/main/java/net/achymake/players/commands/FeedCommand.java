        ...
        if (sender instanceof Player) {
            if (args.length == 0) {
                ...
                if (getDatabase().getCommandCooldown().containsKey("feed-" + player.getUniqueId())) {
                    ...
                    getDatabase().getCommandCooldown().put("feed-" + player.getUniqueId(), System.currentTimeMillis());
                    player.setFoodLevel(20);
                    Players.sendActionBar((Player) sender, "&6Your starvation has been satisfied");
                }
            }
            if (args.length == 1) {
                Player player = (Player) sender;
                if (player.hasPermission("players.command.feed.others")) {
                    ...
                    if (target != null) {
                        ...
                        Players.sendActionBar(target, "&6Your starvation has been satisfied by&f " + player.getName());
        ...
        if (sender instanceof ConsoleCommandSender) {
            if (args.length == 1) {
                ...
                if (target != null) {
                    ...
                    Players.send(commandSender, "You satisfied " + target.getName() + "'s starvation");
                }
            }
        }
        return true;
