    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof ConsoleCommandSender) {
            if (args.length == 0) {
                ...
                Players.send(commandSender, "Usage: /freeze target");
            }
            if (args.length == 1) {
                ConsoleCommandSender commandSender = (ConsoleCommandSender) sender;
                ...
                if (target != null) {
                    getDatabase().setBoolean(target, "settings.frozen", !getDatabase().isFrozen(target));
                    if (getDatabase().isFrozen(target)) {
                        Players.send(commandSender, "You froze " + target.getName());
                    } else {
                        Players.send(commandSender, "You unfroze " + target.getName());
                    }
                } else {
                    OfflinePlayer offlinePlayer = Bukkit.getOfflinePlayer(args[0]);
                    if (getDatabase().exist(offlinePlayer)) {
                        getDatabase().setBoolean(offlinePlayer, "settings.frozen", !getDatabase().isFrozen(offlinePlayer));
                        if (getDatabase().isFrozen(offlinePlayer)) {
                            Players.send(commandSender, "You froze " + offlinePlayer.getName());
                        } else {
                            Players.send(commandSender, "You unfroze " + offlinePlayer.getName());
                        }
                    } else {
