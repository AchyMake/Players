...
public class PlayerBucketEmpty implements Listener {
    ...
    public void onPlayerBucketEmpty(PlayerBucketEmptyEvent event) {
        ...
        if (database.isFrozen(event.getPlayer()) || database.isJailed(event.getPlayer())) {
            ...
            if (config.getBoolean("notification.enable")) {
                if (config.getStringList("notification.bucket-empty").contains(event.getBucket().toString())) {
                    for (Player players : event.getPlayer().getServer().getOnlinePlayers()) {
                        if (players.hasPermission("players.notify.bucket-empty")) {
                            for (String messages : config.getStringList("notification.message")) {
                                players.sendMessage(message.addColor(MessageFormat.format(messages, event.getPlayer().getName(), event.getBucket().toString(), event.getBlock().getWorld().getName(), event.getBlock().getLocation().getBlockX(), event.getBlock().getLocation().getBlockY(), event.getBlock().getLocation().getBlockZ())));
                            }
                        }
                    }
                }
